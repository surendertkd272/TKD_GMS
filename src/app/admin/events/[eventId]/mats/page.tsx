...
export default async function AdminMatsPage({ params }: { params: Promise<{ eventId: string }> }) {
  await requireAdmin();
  ...
  const mats = await db.mat.findMany({
    include: {
      ...
      officials: { where: { eventId, active: true }, select: { id: true, name: true, isJury: true } },
    },
