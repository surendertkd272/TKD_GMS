...
export default async function PrintDrawsPage({
  ...
  const [settings, categories, judgeCount] = await Promise.all([
    getSettings(),
    ...
    db.user.count({ where: { role: 'REFEREE', isJury: true, active: true } }),
  ]);
            ...
            <div className="print-sheet-header">
              <div>
                <p className="text-xs uppercase tracking-wide text-ink-muted">
                  {settings.eventName} · {settings.edition}
                </p>
                <h2 className="text-xl font-semibold text-ink">{category.name}</h2>
                <p className="text-sm text-ink-soft">
                  <span className="num">{category.code}</span> · {category.event} ·{' '}
