...
export default async function PublicHome({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [stats, tally, champion, recent, upcoming, categoryCounts] = await Promise.all([
    ...
    championSchool(event.id, event),
    db.bout.findMany({
      where: { status: 'COMPLETED', category: { eventId: event.id, drawStatus: { in: ['PUBLISHED', 'LOCKED'] } } },
      include: {
    ...
    db.bout.findMany({
      where: {
        ...
        category: { eventId: event.id, drawStatus: { in: ['PUBLISHED', 'LOCKED'] } },
      },
      include: {
    ...
    db.category.groupBy({
      by: ['ageCategory'],
      where: { eventId: event.id, active: true, entries: { some: {} } },
      _count: true,
    }),
  ]);

  const registrationOpen = !event.registrationLocked && event.registrationClosesAt > new Date();
  const showResults = event.resultsPublished;
    ...
    <>
      ...
      <section className="border-b border-surface-line bg-ink">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
          <p className="eyebrow text-white/50">{event.organiser}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {registrationOpen ? (
              <Link href={registerSchoolPath(slug)} className="btn-primary">
                Register your school
              </Link>
            ) : (
              <Link href={eventPath(slug, "results")} className="btn-primary">
                Live results
              </Link>
            )}
            <Link
              href={eventPath(slug, "schedule")}
          ...
          <dl className="mt-8 grid grid-cols-2 gap-x-6 gap-y-4 border-t border-white/10 pt-6 sm:mt-12 sm:gap-x-8 sm:gap-y-6 sm:pt-8 lg:grid-cols-4">
            {[
              ['Venue', event.venue],
              ['Dates', `${fmtDate(event.startDate)} – ${fmtDate(event.endDate)}`],
              [
                'Entries',
                registrationOpen ? `Close ${fmtDate(event.registrationClosesAt)}` : 'Closed',
              ],
              ['Entry fee', `${money(event.feePerParticipant)} per athlete`],
            ].map(([label, value]) => (
              <div key={label}>
          ...
          <div className="space-y-8 lg:col-span-2">
            {showResults && recent.length > 0 && (
              <Card
                ...
                actions={
                  <Link href={eventPath(slug, "results")} className="btn-ghost btn-sm">
                    All results
                  </Link>
                }
                bodyClassName=""
            ...
            {upcoming.length > 0 && (
              <Card
                ...
                actions={
                  <Link href={eventPath(slug, "schedule")} className="btn-ghost btn-sm">
                    Full schedule
                  </Link>
                }
                bodyClassName=""
            ...
            <Card title="Categories" subtitle="Age categories are calculated from date of birth on the reference date.">
              ...
              <p className="mt-5 border-t border-surface-line pt-4 text-xs leading-relaxed text-ink-muted">
                Age is measured on {fmtDate(event.ageReferenceDate)}. Youth is 11 &amp; under, Cadet
                12–14, Junior 15–17.
              </p>
            </Card>
          </div>
          ...
          <div className="space-y-8">
            ...
            {showResults && tally.rows.length > 0 && (
              <Card
                title="Medal tally"
                actions={
                  <Link href={eventPath(slug, "medal-tally")} className="btn-ghost btn-sm">
                    Full table
                  </Link>
                }
                bodyClassName=""
            ...
            <Card title="Draws">
              ...
              <div className="mt-3">
                <StatusBadge status={event.drawsPublished ? 'PUBLISHED' : 'DRAFT'} />
              </div>
              <Link href={eventPath(slug, "results")} className="btn-ghost btn-sm mt-4">
                Browse divisions
              </Link>
            </Card>
          </div>
