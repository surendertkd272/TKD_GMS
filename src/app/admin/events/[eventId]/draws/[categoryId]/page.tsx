...
export default async function AdminCategoryDraw({
  params,
}: {
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          <>
            <span className="num">{category.code}</span> · {category.discipline.toLowerCase()} ·{' '}
        ...
        actions={
          <>
            <Link href={adminPath(eventId, `draws/print?categoryId=${category.id}`)} className="btn-ghost" target="_blank">
              Print
            </Link>
            <Link href={`/results/${category.id}`} className="btn-ghost" target="_blank">
              Public view
            </Link>
            <Link href={adminPath(eventId, 'draws')} className="btn-quiet">
              All draws
            </Link>
          </>
        }
