  ...
  const [participants, schools, stats] = await Promise.all([
    db.participant.findMany({
      ...
      include: {
        ...
        entries: { include: { category: { select: { name: true, event: true } } } },
      },
      orderBy: [{ school: { name: 'asc' } }, { name: 'asc' }],
      take: 400,
    }),
    db.school.findMany({ select: { id: true, name: true, code: true }, orderBy: { name: 'asc' } }),
    db.participant.groupBy({ by: ['ageCategory'], _count: true }),
  ]);

  const byCategory = Object.fromEntries(stats.map((s) => [s.ageCategory, s._count]));
  const unmatchedCount = await db.participant.count({ where: { personRole: 'ATHLETE', entries: { none: {} } } });
                  ...
                  {participants.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td className="whitespace-nowrap text-xs">
                        <Link href={`/admin/schools/${p.schoolId}`} className="hover:text-tkd-red">
                          {p.school.code}
                        </Link>
                      </td>
