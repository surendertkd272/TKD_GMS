...
export default async function SchoolOverview({
  ...
  const [{ school }, settings, { welcome }] = await Promise.all([
    requireSchool(),
    getSettings(),
    searchParams,
  ]);
  ...
  const registrationOpen = !settings.registrationLocked;
  const daysToClose = Math.ceil((settings.registrationClosesAt.getTime() - Date.now()) / 86_400_000);
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        {registrationOpen && daysToClose >= 0 && daysToClose <= 14 && (
          <Notice kind="warn">
            Entries close on <strong>{fmtDate(settings.registrationClosesAt)}</strong> — {daysToClose}{' '}
                      ...
                      {readiness.issues.slice(0, 12).map((issue, i) => (
                        <tr key={`${issue.code}-${i}`}>
                          ...
                          <td className="text-right">
                            {issue.participantId && (
                              <Link
                                href={`/school/participants/${issue.participantId}`}
          ...
          <div className="space-y-6">
            <Card title="Submit for review">
              <p className="mb-4 text-sm leading-relaxed text-ink-soft">
                ...
                participants until registration closes on {fmtDate(settings.registrationClosesAt)}.
              </p>
              <SubmitRegistration
            ...
            <Card title="Institution on file" bodyClassName="card-pad">
              ...
              />
              <Link href="/school/profile" className="btn-ghost btn-sm mt-4">
                Edit details
              </Link>
            </Card>
          </div>
