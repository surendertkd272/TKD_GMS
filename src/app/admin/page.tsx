...
export default async function AdminDashboard() {
  ...
  const [settings, stats, tally, champion, conflicts] = await Promise.all([
    getSettings(),
    eventStats(),
    medalTally(),
    championSchool(),
  ...
  const [pendingSchools, unmatchedAthletes, disputes, recentBouts, drawState, unsentCertificates] = await Promise.all([
    ...
    db.participant.count({ where: { personRole: 'ATHLETE', entries: { none: {} } } }),
    ...
    db.category.groupBy({ by: ['drawStatus'], where: { active: true }, _count: true }),
    db.certificate.count({ where: { emailedAt: null, revoked: false } }),
  ]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Link href="/admin/draws" className="btn-ghost">
              Draws
            </Link>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {pendingSchools.length > 0 && (
              <Notice kind="warn">
                ...
                <Link href="/admin/schools?status=PENDING" className="font-medium underline">
                  Review now
                </Link>
              </Notice>
            )}
            ...
            {disputes > 0 && (
              <Notice kind="error">
                ...
                official.{' '}
                <Link href="/admin/live" className="font-medium underline">
                  Open live control
                </Link>
              </Notice>
            )}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <Card
              ...
              actions={
                <Link href="/admin/live" className="btn-ghost btn-sm">
                  Live control
                </Link>
              }
              bodyClassName=""
          ...
          <div className="space-y-6">
            <Card
              ...
              subtitle={`Gold ${settings.pointsGold} · Silver ${settings.pointsSilver} · Bronze ${settings.pointsBronze}`}
              bodyClassName="card-pad"
            >
              {!champion ? (
                  ...
                  {pendingSchools.map((school) => (
                    <li key={school.id} className="flex items-center justify-between gap-3 px-5 py-3">
                      <div className="min-w-0">
                        <Link href={`/admin/schools/${school.id}`} className="block truncate text-sm font-medium text-ink hover:text-tkd-red">
                          {school.name}
                        </Link>
