...
export default async function PrintScoringSheetsPage({
  ...
  const [settings, bouts] = await Promise.all([
    getSettings(),
    db.bout.findMany({
      where: {
        status: { not: 'BYE' },
        category: { event: 'KYORUGI', ...(categoryId ? { id: categoryId } : {}) },
      },
      include: {
            ...
            <div className="print-sheet-header">
              <div>
                <p className="text-xs uppercase tracking-wide text-ink-muted">
                  {settings.eventName} · {settings.edition} · Official scoring sheet (paper backup)
                </p>
