...
export default async function SchoolPaymentPage() {
  ...
  return (
    <>
      <PageHeader
        title="Entry fee"
        subtitle={`₹${settings.feePerParticipant} per athlete. Coaches, officials and volunteers are accredited free of charge.`}
      />

      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-3">
            <Stat label="Athletes entered" value={athletes} hint={`× ${money(settings.feePerParticipant)}`} />
        ...
        <div className="space-y-6">
          ...
          <Card title="Where to pay" bodyClassName="card-pad">
            <KeyValue
              rows={[
                ['Payable to', settings.organiser],
                ['Event', `${settings.eventName} ${settings.edition}`],
                ['Per athlete', money(settings.feePerParticipant)],
