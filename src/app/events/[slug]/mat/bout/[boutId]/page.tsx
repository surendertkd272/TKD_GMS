...
export default async function MatBoutPage({ params }: { params: Promise<{ boutId: string }> }) {
  const [{ session, user, event }, { boutId }] = await Promise.all([requireReferee(), params]);
    ...
    return (
      <>
        ...
        <Notice kind="error">
          ...
          <Link href={matPath(event.slug)} className="font-medium underline">
            Back to your queue
          </Link>
        </Notice>
      </>
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link href={matPath(event.slug)} className="btn-quiet">
            Back to queue
          </Link>
        }
      />
      ...
      <div className="space-y-6">
        ...
        {bout.category.discipline === 'POOMSAE' ? (
          <Notice kind="info">
            This is a Poomsae category — scoring happens on the{' '}
            <Link href={matPath(event.slug, `poomsae/${bout.categoryId}`)} className="font-medium underline">
