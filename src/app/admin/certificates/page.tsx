...
export default async function AdminCertificatesPage() {
  ...
  const [certificates, finalizedCategories, schools, stats] = await Promise.all([
    ...
    db.school.findMany({
      where: { participants: { some: { certificates: { some: { revoked: false } } } } },
      select: { id: true, name: true, code: true, contactEmail: true },
      orderBy: { name: 'asc' },
    }),
    db.certificate.groupBy({ by: ['type'], where: { revoked: false }, _count: true }),
  ]);

  const [totalCerts, unsent] = await Promise.all([
    db.certificate.count({ where: { revoked: false } }),
    db.certificate.count({ where: { revoked: false, emailedAt: null } }),
  ]);
