 ...
 */
export default async function ParticipantPublicPage({ params }: { params: Promise<{ code: string }> }) {
  const [settings, { code }] = await Promise.all([getSettings(), params]);
    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="space-y-6">
        {!visible ? (
          ...
          <Notice kind="ok">
            <strong>Accreditation valid</strong> for {settings.eventName} {settings.edition}. Revision{' '}
                  ...
                  {participant.entries.map((entry) => (
                    <tr key={entry.id}>
                      ...
                      <td className="text-xs uppercase tracking-wide text-ink-muted">
                        {entry.category.event.toLowerCase()}
                      </td>
                      ...
                      <td className="text-right">
                        {entry.category.drawStatus === 'PUBLISHED' || entry.category.drawStatus === 'LOCKED' ? (
                          <Link href={`/results/${entry.categoryId}`} className="btn-ghost btn-sm">
                            Bracket
                          </Link>
                        ) : null}
                      </td>
                ...
                <tbody>
                  {participant.certificates.map((cert) => (
                    <tr key={cert.id}>
                      ...
                      <td className="text-right">
                        <Link href={`/verify/${cert.certNo}`} className="btn-ghost btn-sm">
                          Verify
                        </Link>
                      </td>
                    </tr>
