...
export default async function AdminLivePage({
  ...
  const completed = await db.bout.count({ where: { status: 'COMPLETED' } });
      ...
      <div className="space-y-6">
                ...
                <li key={bout.id} className="px-5 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/admin/live?bout=${bout.id}`} className="btn-ghost btn-sm">
                        Override result
                      </Link>
        ...
        {selected && (
          <Card
            ...
            actions={
              <Link href="/admin/live" className="btn-quiet btn-sm">
                Close
              </Link>
            }
          >
                ...
                <ul className="divide-y divide-surface-line">
                  {mat.bouts.map((bout, idx) => (
                    <li key={bout.id} className={`px-4 py-3 ${idx === 0 ? 'bg-surface-sunk/50' : ''}`}>
                      ...
                      <Link href={`/admin/live?bout=${bout.id}`} className="btn-quiet btn-sm mt-1.5 !px-0">
                        Override →
                      </Link>
                    </li>
                  ))}
                  ...
                  {poomsaePending.map((category) => (
                    <tr key={category.id}>
                      ...
                      <td className="text-right">
                        <Link href={`/admin/draws/${category.id}`} className="btn-ghost btn-sm">
                          Review &amp; finalise
                        </Link>
                      </td>
                    </tr>
        ...
        {mats.length === 0 && (
          <Empty
            ...
            action={
              <Link href="/admin/mats" className="btn-primary btn-sm">
                Manage mats
              </Link>
            }
          />
