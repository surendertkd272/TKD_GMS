...
export default async function MedalTallyPage({
  ...
  const filter = {
    event: params.event as 'KYORUGI' | 'POOMSAE' | undefined,
    ageCategory: params.cat,
    gender: params.gender,
  };
  const { rows, totals } = await medalTally(filter);
      ...
      <div className="space-y-6">
        ...
        {championRow && !filtered && (
          <Card
            ...
            subtitle={`Weighted points — gold ${settings.pointsGold}, silver ${settings.pointsSilver}, bronze ${settings.pointsBronze}.`}
          >
        ...
        <Card bodyClassName="card-pad">
          <form className="grid gap-3 sm:grid-cols-[auto_auto_auto_auto_1fr]">
            ...
            {filtered && (
              <Link href="/medal-tally" className="btn-quiet justify-self-start sm:justify-self-end">
                Clear filters
              </Link>
            )}
          </form>
