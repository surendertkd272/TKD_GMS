...
export default async function AdminSchedulePage({
  ...
  const [bouts, mats, referees, conflicts, categories] = await Promise.all([
    ...
    db.mat.findMany({ where: { eventId }, orderBy: { sortOrder: 'asc' } }),
    db.user.findMany({ where: { eventId, role: 'REFEREE', active: true }, orderBy: { name: 'asc' } }),
    detectScheduleConflicts(eventId),
    db.category.findMany({
      where: { eventId, bouts: { some: {} } },
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link href={adminPath(eventId, 'schedule/print')} className="btn-ghost" target="_blank">
      ...
      <div className="space-y-5">
        ...
        <AutoScheduleForm defaultStart={toDateTimeInput(event.startDate)} />
