...
export async function generateMetadata({
  ...
  const category = event
    ? await db.category.findFirst({ where: { id: categoryId, eventId: event.id }, select: { name: true } })
    : null;
...
export default async function PublicCategoryPage({
  ...
  if (!isPublic || !event.resultsPublished) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <Link href={eventPath(slug, 'results')} className="btn-ghost mt-4">
          Back to results
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <PageHeader
        ...
        subtitle={
          <>
            {category.discipline === 'KYORUGI' ? 'Kyorugi — single elimination' : 'Poomsae — ranked scoring'} ·{' '}
      ...
      <div className="space-y-6">
        ...
        {category.discipline === 'KYORUGI' && category.bouts.length > 0 && (
