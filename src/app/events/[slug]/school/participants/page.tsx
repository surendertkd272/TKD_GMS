...
export default async function ParticipantsPage({
  ...
  const [{ school, event }, params] = await Promise.all([requireSchool(), searchParams]);
  ...
  return (
    <>
      <PageHeader
        title="Participants"
        subtitle={`${total} entered for ${school.name}. Edit freely until registration closes on ${fmtDate(event.registrationClosesAt)}.`}
        actions={
          <>
            <Link href={schoolPath(event.slug, 'bulk-upload')} className="btn-ghost">
              Bulk CSV upload
            </Link>
            ...
            action={
              total === 0 ? (
                <div className="flex gap-2">
                  <Link href={schoolPath(event.slug, 'participants/new')} className="btn-primary btn-sm">
                    Add participant
                  </Link>
                  <Link href={schoolPath(event.slug, 'bulk-upload')} className="btn-ghost btn-sm">
                    Bulk upload
                  </Link>
                </div>
              ) : (
                <Link href={schoolPath(event.slug, 'participants')} className="btn-ghost btn-sm">
                  Clear filters
                </Link>
              )
            }
