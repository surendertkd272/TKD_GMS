...
export default async function PublicSchedulePage({
  ...
  const [mats, schools, bouts] = await Promise.all([
    db.mat.findMany({ where: { active: true }, orderBy: { sortOrder: 'asc' } }),
    db.school.findMany({ where: { status: 'APPROVED' }, select: { id: true, name: true, code: true }, orderBy: { name: 'asc' } }),
    db.bout.findMany({
      where: {
        status: { not: 'BYE' },
        category: { drawStatus: { in: ['PUBLISHED', 'LOCKED'] } },
        ...(params.mat ? { matId: params.mat } : {}),
        ...(params.cat ? { category: { ageCategory: params.cat, drawStatus: { in: ['PUBLISHED', 'LOCKED'] } } } : {}),
        ...(params.school
          ? {
              OR: [
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <PageHeader
        title="Schedule"
        subtitle={`${settings.venue} · ${fmtDate(settings.startDate)} – ${fmtDate(settings.endDate)}. Filter by mat, school or age category.`}
      />
