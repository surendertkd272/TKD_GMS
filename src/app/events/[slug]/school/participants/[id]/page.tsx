...
export default async function EditParticipantPage({ params }: { params: Promise<{ id: string }> }) {
  const [{ school, event }, { id }] = await Promise.all([requireSchool(), params]);
    ...
    <>
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link href={schoolPath(event.slug, 'participants')} className="btn-quiet">
              Back to list
            </Link>
          </>
        }
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
          ...
          {inLiveDraw && !event.registrationLocked && (
            <Notice kind="info">
          ...
          eventSlug={event.slug}
            mode="edit"
            readOnly={event.registrationLocked}
            ...
            values={{
              ...
              events: [...new Set(participant.entries.map((e) => e.category.discipline))],
